...
redownload <- FALSE # should we re-download from BioC's AWS servers?
keep.files <- TRUE # should we keep BAM files afterwards?
clear.memory <- FALSE # should we clear objects from memory?
```
...
All reads mapped to intervals in the blacklist will be ignored during processing in `r Biocpkg("csaw")`.
...
This is done using the `liftOver` function in the `r Biocpkg("rtracklayer")` package [@lawrence2009rtracklayer].
...
This is identified from Figure ((ccfplot)) and is used as the average fragment length for this analysis.
...
The effect of the fold-change threshold is examined visually in Figure ((bghistplot)).
...
Here, a trended bias is present between libraries in Figure ((trendplot)).
...
```{r trendplot, fig.cap="Abundance-dependent trend in the log-fold change between two H3K9ac libraries (mature B over pro-B), across all windows retained after filtering."}
...
Once the offsets are applied to adjust the log-fold changes, the trend is eliminated from the plot (Figure ((normplot))).
...
```{r normplot, fig.cap="Effect of non-linear normalization on the trended bias between two H3K9ac libraries. Normalized log-fold changes are shown for all windows retained after filtering."}
...
The NB dispersion trend is visualized in Figure ((bcvplot)) as the biological coefficient of variation (BCV), i.e., the square root of the NB dispersion.
...
In Figure ((bcvplot)), the range of abundances after filtering is such that the plateau has already been reached.
...
The instability of the QL dispersion estimates is reduced by squeezing the estimates towards an abundance-dependent trend (Figure ((qlplot))).
...
In Figure ((mdsplot)), strong separation in the first dimension is observed between libraries from different cell types.
...
This represents a simple DB event where the entire region changes in one direction (Figure ((simplebroadplot))).
...
```{r simplebroadplot, fig.width=8, fig.height=6, fig.cap="Coverage tracks for a simple DB event between pro-B and mature B cells, across a broad region in the H3K9ac data set. Read coverage for each library is shown as a per-million value at each base."}
collected <- list()
...
This region contains a bidirectional promoter where different genes are marked in the different cell types (Figure ((complexplot))).
...
```{r complexplot, fig.width=8, fig.height=6, fig.cap="Coverage tracks for a complex DB event in the H3K9ac data set, shown as per-million values."}
collected <- list()
for (i in 1:length(bam.files)) { 
    reads <- extractReads(bam.file=bam.files[i], cur.region, param=param)
    cov <- as(coverage(reads)/lib.sizes[i], "GRanges")
...
Marking is increased for mature B cells within a 500 bp region (Figure ((simplesharpplot))), which is sharper than the changes in the previous two examples.
...
```{r simplesharpplot, fig.width=8, fig.height=6, fig.cap="Coverage tracks for a sharp and simple DB event in the H3K9ac data set, shown as per-million values."}
collected <- list()
for (i in 1:length(bam.files)) { 
    reads <- extractReads(bam.file=bam.files[i], cur.region, param=param)
    cov <- as(coverage(reads)/lib.sizes[i], "GRanges")
...
The effect of normalization is visualized with some mean-difference plots between pairs of libraries (Figure ((compoplot))).
...
```{r compoplot, fig.width=12, fig.height=6, fig.cap="Mean-difference plots for the bin counts, comparing library 4 to all other libraries. The red line represents the log-ratio of the normalization factors between libraries."}
...
This is illustrated in Figure ((mdsplot2)) where the WT libraries are clearly separated in both dimensions of the MDS plot.
...
```{r mdsplot2, fig.cap="MDS plot with two dimensions for all libraries in the CBP data set. Libraries are labelled and coloured according to the genotype. A larger top set of windows was used to improve the visualization of the genome-wide differences between the WT libraries."}
...
In Figure ((tfplot)), two adjacent sites are present at the *Gbe1* promoter, both of which exhibit increased binding in the WT genotype.
Coverage is also substantially different between the WT replicates, consistent with the presence of a batch effect.

```{r tfplot, fig.height=6, fig.width=8, fig.cap="Coverage tracks for TF binding sites that are differentially bound in the WT (top two tracks) against the KO (last two tracks). Blue and red tracks represent forward- and reverse-strand coverage, respectively, on a per-million scale (capped at 5 in SRR1145788, for visibility)."}
collected <- list()
lib.sizes <- filtered.data$totals/1e6
for (i in 1:length(bam.files)) { 
    reads <- extractReads(bam.file=bam.files[i], cur.region, param=param)
